...
\section{SUMMARY}
\textbf{5+} years experiences in data science and machine learning, a well-trained scientist in mathematical optimization and algorithm development, passionate presenter with extensive knowledge in numerical methods for solving large scale statistical learning problems. %Highly analytical, focused and personable, with broad interests in quantitative research, technology and consulting
...
\section{WORK EXPERIENCE} \parskip 11 pt
	\textbf{Honeywell International Inc.} \quad\quad\textbf{Data Scientist, Data Science CoE}\hfill Oct. 2016 - Present
		\begin{itemize}
			\item Lead data engineers to cleanup over 1 TB raw power consumption data consists of 1.5 million text files on \textbf{Azure} cluster through creating reusable \textbf{pyspark} library and establishing \textbf{Hive} database  
			\item Selected features from weather data, day of week and partial ACF, implemented a \textbf{RBF Neural Network} model with \textbf{R} for short-term power load \textbf{time series} prediction     
			\item Utilized external property data, carried out univariate analysis and segregated population into 15 micro segments by maximizing the margin of average power consumption 
			\item Trained unusual usage model on segment level on cluster using \textbf{sparklyr} for specifying abnormal observations to convert extremely imbalanced population into balanced training dataset
			%\item Mentoring a junior data scientist on coding and determining optimal threshold for filtering
			\item Using water usage, smart meter events/status data, trained a energy theft detection model
			\item Received Bravo recognition award, heavily interacted with implementation team, data scientists and business via advising, mentoring and presenting
	...
	\textbf{Chubb Corporation}\quad \textbf{Claim Actuarial Modeler, Advanced Analytics}\hfill May 2015 - Oct. 2016
		\begin{itemize} \itemsep -1 pt
			...
			\item  Conducted \textbf{social network analysis} and established network-revenue model for underwriters with \textbf{iGraph}, helped lower performs to create more revenue by a margin of 5\%
			\item Created \textbf{SAS} codes for monitoring and tracking models performance in production system
...
%			\item Programmed the effective plotting interface for understanding and interpreting modeling work
			...
			\item Implemented the web service using \textbf{Linux}, \textbf{Apache} web server, along with \textbf{MySQL}, \textbf{Python} and \textbf{R} for server-side, \textbf{Javascript} and \textbf{HTML5} for client-side programming %Greatly reduced the overhead of communication between clinicians and  pharmacometricians					
    ...
    DHS - Rutgers CoE (CCICADA)}\quad\quad\textbf{Advanced Analytics Researcher}  \hfill Jan.-Jun. 2013
        \begin{itemize}\itemsep -1 pt
            \item Captured and analyzed complex information given by U.S. Coast Guard to establish the optimal boat-allocation integer programming model with \textbf{Xpress-Mosel}%that generates various schemes with different parameters
            %\item Presented the briefings to upper management monthly and drafted the guidance on the project
            \item Recommended the best boat-sharing plan for U.S. Coast Guard to cover required mission hours under tight budget and presented the analysis and delivered the product to the C-level management %monthly and drafted the guidance on the project. 
        \end{itemize}\vspace{-\topsep}
...
%               \end{itemize}
    ...
    \begin{itemize}\itemsep -1 pt
    	\item Tested extensively the Bioprocess Data Collection System Data Mart(BDCSDM) with \textbf{SQL} %and enhanced the data visualization application by adding new practical features with \textbf{.NET}
    	\item  Automated the visualization tool to be database driven, eliminating maintenance overhead %for five filtration experiments
